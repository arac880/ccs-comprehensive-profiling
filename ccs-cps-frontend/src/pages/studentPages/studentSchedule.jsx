...
export default function StudentSchedule() {
  ...
  const [currentNav, setCurrentNav] = useState("Schedule");
  ...
  // ── EASY DATA ENTRY (No more guessing pixels!) ──
  // Use 24-hour format for startHour (e.g., 2:30 PM = startHour: 14, startMin: 30)
  const scheduleData = [
  ...
  // ── Auto-Pixel Calculator Helper ──
  const getCardPosition = (startHour, startMin, durationHours) => {
    let top = 40; // Starts after MORNING spacer
    ...
    if (startTimeDecimal < 12) {
      ...
      if (endTimeDecimal > 12) height += 40; // Crosses into afternoon, add spacer height
    } else {
      top += (6 * 60) + 40 + ((startTimeDecimal - 12) * 60); // Morning slots + Afternoon spacer + Afternoon slots
    }
          ...
          <div className={schedStyles.calendarBody}>
            ...
            <div className={schedStyles.timeColumn}>
              <div className={schedStyles.periodDivider}>
                <span className={schedStyles.periodLine}></span> MORNING <span className={schedStyles.periodLine}></span>
              </div>
              ...
              <div className={schedStyles.periodDivider}>
                 <span className={schedStyles.periodLine}></span> AFTERNOON <span className={schedStyles.periodLine}></span>
              </div>
            ...
            {/* Right Columns: Grid Lines & Classes */}
            {days.map(day => (
              <div key={day} className={schedStyles.dayColumn}>
                ...
                {timeSlots.slice(0, 12).map((_, idx) => (
                  <div key={`am-grid-${idx}`} className={schedStyles.gridLine}></div>
                ))}

                {/* AFTERNOON GRID LINES */}
                <div className={schedStyles.periodSpacer}></div>
                {timeSlots.slice(12, 30).map((_, idx) => (
                  <div key={`pm-grid-${idx}`} className={schedStyles.gridLine}></div>
                ))}
                ...
                {scheduleData.filter(c => c.day === day).map((cls, idx) => {
                  ...
                  return (
                    <div 
                      ...
                      style={{ top: styleProps.top, height: styleProps.height, backgroundColor: cls.color }}
                    >
  ...
  if (isMobile) {
    return (
      <>
        ...
        <main className={layoutStyles.mobileMain}>{scheduleUI}</main>
      </>
    );
  }
