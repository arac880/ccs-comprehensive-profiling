      ...
      try {
        ...
        const mySchedules = allSchedules.filter((s) => {
          ...
          if (!sProg || sProg === dbProg) matchProg = true;
            ...
            dbProg.toLowerCase().includes("information") &&
            sProg.toLowerCase().includes("it")
          )
            matchProg = true;
          else if (
            dbProg.toLowerCase().includes("computer") &&
            sProg.toLowerCase().includes("cs")
          )
            matchProg = true;
          ...
          const matchYear =
            !sYear ||
            sYear === dbYear ||
            (dbYearDigit && dbYearDigit === sYearDigit);
          ...
          const matchSec =
            !sSec ||
            sSec === dbSec ||
            (dbSecLetter && dbSecLetter === sSecLetter);
        ...
        setError(
          "Failed to load your schedule. Please ensure you are logged in properly.",
        );
      } finally {
        setLoading(false);
      }
