...
import { FaPercent, FaBook, FaFileLines, FaPesoSign, FaClipboardCheck } from "react-icons/fa6";
  ...
  useEffect(() => {
    const fetchClearance = async () => {
      try {
        ...
        const res = await fetch(`http://localhost:5000/api/students/${currentId}`);
      ...
      <div className={styles.statusBanner}>
        <div className={styles.statusLeft}>
          ...
          <span className={styles.statusSubText}>
            {isOverallCleared ? "You have met all requirements." : "Action required on pending items."}
          </span>
        </div>
      ...
      <div className={styles.timelineWrapper}>
        ...
        <div className={styles.timelineRow}>
          <div className={styles.timelineNodeBox}>
            <div className={`${styles.timelineNode} ${pathway.gradeStatus.isCleared ? styles.nodeCleared : styles.nodePending}`}>
              {pathway.gradeStatus.isCleared ? <MdCheckCircle size={24} /> : <MdCancel size={24} />}
            </div>
            ...
            <div className={styles.widgetCard}>
              ...
              <div className={styles.widgetBody}>
                <div className={styles.bodyLeft}>
                  <h4 className={pathway.gradeStatus.isCleared ? styles.statusPassText : styles.statusFailText}>
                    {pathway.gradeStatus.statusText}
                  </h4>
                  <p className={styles.subtext}>{pathway.gradeStatus.subtext}</p>
                </div>
                {pathway.gradeStatus.missingItems && pathway.gradeStatus.missingItems.length > 0 && (
                  <div className={styles.missingItemsWrapper}>
                    {pathway.gradeStatus.missingItems.map((item, idx) => (
                      <div key={idx} className={styles.missingItem}>
                        <MdCancel size={16} /><span>{item}</span>
                      </div>
                    ))}
                  </div>
                )}
        ...
        <div className={styles.timelineRow}>
          <div className={styles.timelineNodeBox}>
            <div className={`${styles.timelineNode} ${pathway.accountStatus.isCleared ? styles.nodeCleared : styles.nodePending}`}>
              {pathway.accountStatus.isCleared ? <MdCheckCircle size={24} /> : <MdCancel size={24} />}
            </div>
            <div className={styles.timelineLine}></div>
          </div>
          ...
          <div className={styles.timelineContent}>
            <div className={styles.widgetCard}>
              ...
              <div className={styles.widgetBody}>
                <div className={styles.bodyLeft}>
                  <h4 className={pathway.accountStatus.isCleared ? styles.statusPassText : styles.statusFailText}>
                    {pathway.accountStatus.statusText}
                  </h4>
                  <p className={styles.subtext}>{pathway.accountStatus.subtext}</p>
                </div>
                {pathway.accountStatus.tags && pathway.accountStatus.tags.length > 0 && (
                  <div className={styles.extraDetailsRow}>
                    {pathway.accountStatus.tags.map((tag, idx) => {
                      ...
                      return (
                        <span key={idx} className={isPending ? styles.missingItem : styles.successTag}>
                          {isPending ? <MdCancel size={14} style={{ marginRight: '4px' }} /> : "✓"} {tag}
                        </span>
                      );
                    })}
                  </div>
        ...
        <div className={styles.timelineRow}>
          <div className={styles.timelineNodeBox}>
            <div className={`${styles.timelineNode} ${pathway.libraryStatus.isCleared ? styles.nodeCleared : styles.nodePending}`}>
              {pathway.libraryStatus.isCleared ? <MdCheckCircle size={24} /> : <MdCancel size={24} />}
            </div>
            <div className={styles.timelineLine}></div>
          </div>
          ...
          <div className={styles.timelineContent}>
            <div className={styles.widgetCard}>
              ...
              <div className={styles.widgetBody}>
                <div className={styles.bodyLeft}>
                  <h4 className={pathway.libraryStatus.isCleared ? styles.statusPassText : styles.statusFailText}>
                    {pathway.libraryStatus.statusText}
                  </h4>
                  <p className={styles.subtext}>{pathway.libraryStatus.subtext}</p>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className={styles.timelineRow}>
          <div className={styles.timelineNodeBox}>
            <div className={`${styles.timelineNode} ${pathway.requirements.isCleared ? styles.nodeCleared : styles.nodePending}`}>
              {pathway.requirements.isCleared ? <MdCheckCircle size={24} /> : <MdCancel size={24} />}
            </div>
          </div>
          ...
          <div className={styles.timelineContent}>
            <div className={styles.widgetCard}>
              ...
              <div className={styles.widgetBody}>
                <div className={styles.bodyLeft}>
                  <h4 className={pathway.requirements.isCleared ? styles.statusPassText : styles.statusFailText}>
                    {pathway.requirements.statusText}
                  </h4>
                  <p className={styles.subtext}>{pathway.requirements.subtext}</p>
                </div>
                {pathway.requirements.tags && pathway.requirements.tags.length > 0 && (
                  <div className={styles.extraDetailsRow}>
                    {pathway.requirements.tags.map((tag, idx) => {
                       const isMissing = tag.toLowerCase().includes("missing");
                       return (
                         <span key={idx} className={isMissing ? styles.missingItem : styles.successTag}>
                           {isMissing ? <MdCancel size={14} style={{ marginRight: '4px' }} /> : "✓"} {tag}
                         </span>
                       );
                    })}
                  </div>
