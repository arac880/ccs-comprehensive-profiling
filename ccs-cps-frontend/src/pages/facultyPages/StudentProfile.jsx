        ...
        {!isLoading && !error && student && (
          <>
            <div className={styles.heroCard}>
              ...
              {isEditing ? (
                <select name="status" value={editData.status || "Regular"} onChange={handleChange} style={{ padding: "6px 12px", borderRadius: "12px", border: "2px solid #e8ddd5", fontSize: "12px", fontWeight: 600, color: "#1a1a1a", backgroundColor: "#fdfaf7", cursor: "pointer", outline: "none", minWidth: "100px", flexShrink: 0 }}>
                  <option value="Regular">Regular</option>
                  <option value="Irregular">Irregular</option>
                </select>
              ) : (
                <span className={`${styles.badge} ${styles[STATUS_CLASS[student.status] || "badgeActive"]}`}>{student.status || "Regular"}</span>
              )}
            </div>
